...
function showMenu(){
  ...
  // Book-embedded Study Questions (BOOKQ, shown under a chapter's notes) are
  // deliberately NOT folded into totalQ or the Answered/Accuracy dashboard
  // stats below — they're a separate quick-practice pool, not part of the
  // part's real question bank, and counting them here inflated both the
  // denominator and the accuracy total with questions the student never
  // chose to "take" as a quiz. They still get their own per-chapter
  // "X/Y correct" tally under the notes view (bookqHTMLScoped).
  ...
  // Shared across every fold() call below so a question answered in more
  // than one pool — e.g. a chapter-bank question that's also in a mock exam
  // — only counts once toward these dashboard totals, same reasoning as
  // totalQ above. Whichever pool gets folded in first "wins" the tie for
  // right/wrong; pools are folded in a fixed order (exams, then chapter
  // bank, extra, Gleim). BOOKQ is deliberately not folded in at all — see
  // the comment by totalQ above.
  var _countedQ={};
  ...
  // BOOKQ (chapter-notes Study Questions) intentionally excluded from these
  // dashboard totals — see the comment by totalQ above.
